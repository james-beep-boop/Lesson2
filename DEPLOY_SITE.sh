...
REMOTE_PHP_BIN="${REMOTE_PHP_BIN:-/usr/local/php85/bin/php}"
...
(cd "$_BUNDLE" && composer install --no-dev --optimize-autoloader --no-interaction --quiet --no-scripts)
...
echo "    PHP:     $($PHP_BIN -r 'echo PHP_VERSION;') (local) / $REMOTE_PHP_BIN (DreamHost target)"
